...
//  Forward backward procedure, compute likelihood, gradient wrt parameter W and gradient wrt input X
...
void forward_backward_bp (	double *X,
				double *lab_tok,
...
void forward_backward_bp_sequence (	double *X,
					double *lab_tok,
					int T,
					int nbclass,int dim,
					double *w,
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	/* input variables */
	X	= (double*)mxGetData(prhs[0]); nbtok = mxGetN(prhs[0]); 
	lab_tok	= (double*)mxGetData(prhs[1]);
	beg_seq	= (double*)mxGetData(prhs[2]); nbseq = mxGetM(prhs[2]); /* M lines, N columns */
...
void forward_backward_bp (	double *X,
	...
	*F = 0.0;
	for(i=0;i<nbseq;i++) {
		...
		forward_backward_bp_sequence	(X+T0*dim,
						lab_tok+T0,
						T,nbclass,dim,w,
						F,Grad,GX+T0*dim);
	}
...
static  double time_wf=0.0,time_Forward=0.0,time_Backward=0.0,time_grad=0.0,time_total=0.0,last_time=0.0;
	...
	*F += Z;
	...
	for(t=0;t<T;t++) {
		...
		*F -= WF[y][t];
		for (y=0;y<N;y++) {
			if ((y==lab_tok[t]-1)||(fabs(coef[y][t])>SMALL_PROBABILITY)) {
				/* local feature */
				idx0 = N*N + y*dim;
#if FLAG_BLAS
				daxpy_(&dim,coef[y]+t,  X+L0,&incx,Grad+idx0,&incy);
				daxpy_(&dim,coef[y]+t,w+idx0,&incx,GX+L0    ,&incy);
#else
				for (d=0;d<dim;d++) {
				  Grad[idx0 + d] += coef[y][t] * X[L0+d];
				  GX[L0 + d] += coef[y][t] * w[idx0+d];
				}
#endif
				
/*
				for (i=0;i<dim;i++) {
					Grad[idx0+i] += X[L0+i]*coef[y][t];
					GX[L0+i] += coef[y][t]*w[idx0+i];
				}
*/
				/* transition feature */
				if (t>=1) {
