...
pairwise = nchoosek(UniqueClass,2);

sig = [];
count = 0;
for k=1:size(pairwise,1)
% find indexes for both classes
ClassA =  any( bsxfun(@eq, Y, pairwise(k,1)) , 2 );
ClassB = any( bsxfun(@eq, Y, pairwise(k,2)) , 2 );
if(sum(ClassA) > 0 && sum(ClassB)>0)
nA     = sum(double(ClassA));
nB     = sum(double(ClassB));

% calculate significances
error = abs(mean(X(ClassA,:),1) - mean(X(ClassB,:),1));
ind = error < 1e-5;
error(:,ind) = 0;
